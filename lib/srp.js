...
(function() {
  ...
  SRP = (function() {
    ...
    SRP.prototype.M1 = function(options) {
      var A, B, I, K, S, hash, result;
      A = options.A;
      B = options.B;
      I = options.I;
      K = options.K;
      S = options.S;
      hash = crypto.createHash(this.params.hash);
      if ((!!S) && (!!I)) {
          var hN = crypto.createHash(this.params.hash).update(transform.bigInt.toBuffer(this.params.N)).digest();
          var hG = crypto.createHash(this.params.hash).update(transform.bigInt.toBuffer(this.params.g)).digest();
          var hU = crypto.createHash(this.params.hash).update(I).digest();
          for (var i = 0; i < hN.length; i++) hN[i] ^= hG[i];
          hash.update(hN).update(hU).update(S);
      }
      result = hash.update(A).update(B).update(K).digest();
      return result;
    };
